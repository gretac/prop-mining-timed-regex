...
\begin{algorithm}[h]
    \caption{Timed Regular Expression Mining without Negation}\label{alg:the_alg}
    \begin{algorithmic}[1]
...
\begin{algorithm}[h]
    \caption{Timed Regular Expression Mining with Negation}\label{alg:the_alg_neg}
    \begin{algorithmic}[1]
     \Require  Given a set of traces with unique event \emph{i} and a TRE pattern
     \Ensure Parse TRE and formulate a finite Timed Automaton $\rightarrow$ A
     \State Initialize the incidence matrix of size $\Sigma^p$
...
In Algorithm ~\ref{alg:the_alg}, $\Sigma$ is the events alphabet and $p$ is the number of event variables in the TRE template which do not have negation. $A$ denotes the timed automaton and $FS$ and $ES$ denote the final and error states of the timed automaton respectively. $S$ and $C$ denote the \emph{support} and \emph{confidence} metrics of the TRE instances on the given trace.

In contrast, the algorithm ~\ref{alg:the_alg_neg} evaluates the TREs with negations and has an additional for loop to iterate over all possible permutations.
